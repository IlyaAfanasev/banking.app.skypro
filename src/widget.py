...
def get_mask_account_card(account_info: str) -> str:
    ...
    match = re.search(r"([a-zA-Zа-яА-Я\s]+)([0-9]+)", account_info)
    ...
    try:
        if len(number_info) == 16:
            return f"{account_name}{masks.get_mask_card_number(number_info)}"

        elif len(number_info) == 20:
            return f"{account_name}{masks.get_mask_account(number_info)}"
